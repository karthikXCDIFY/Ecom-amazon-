import React from "react";
import "../Fotter.scss";
function Fotter() {
  return (
    <div>
      <div className="fotter-main">
        <main>
          <section className="account-details" />
          <div className="account-section-Men">
            <h2>Men</h2>
            <ul>
              <li>
                <a href="#">Sherwani</a>
              </li>
              <li>
                <a href="#">Track Pants</a>
              </li>
              <li>
                <a href="#">Blazers</a>
              </li>
              <li>
                <a href="#">Sweaters For Men</a>
              </li>
              <li>
                <a href="#">Men Wedding Dresses</a>
              </li>
              <li>
                <a href="#">Kurta Pajama</a>
              </li>
              <li>
                <a href="#">Raincoats</a>
              </li>
              <li>
                <a href="#">Shorts</a>
              </li>
              <li>
                <a href="#">Trousers Waistcoat</a>
              </li>
              <li>
                <a href="#">Inner Wear</a>
              </li>
              <li>
                <a href="#">Nightwear</a>
              </li>
              <li>
                <a href="#">Jeans</a>
              </li>
            </ul>
          </div>
          <div className="account-section-women">
            <h2>Women</h2>
            <ul>
              <li>
                <a href="#">Sarees</a>
              </li>
              <li>
                <a href="#">Kurtas</a>
              </li>
              <li>
                <a href="#">Kurti Sets</a>
              </li>
              <li>
                <a href="#">Dresses</a>
              </li>
              <li>
                <a href="#">Tops</a>
              </li>
              <li>
                <a href="#">Tunics</a>
              </li>
              <li>
                <a href="#">Skirts</a>
              </li>
              <li>
                <a href="#">Palazzos</a>
              </li>
              <li>
                <a href="#">Leggings</a>
              </li>
              <li>
                <a href="#">Shorts</a>
              </li>
              <li>
                <a href="#">Trousers</a>
              </li>
              <li>
                <a href="#">Waistcoat</a>
              </li>
              <li>
                <a href="#">Nightwear</a>
              </li>
            </ul>
          </div>
          <div className="account-section-Kids">
            <h2>Kids</h2>
            <ul>
              <li>
                <a href="#">Boys Clothing</a>
              </li>
              <li>
                <a href="#">Girls Clothing</a>
              </li>
              <li>
                <a href="#">Toys</a>
              </li>
              <li>
                <a href="#">Baby Care</a>
              </li>
              <li>
                <a href="#">Kids Footwear</a>
              </li>
            </ul>
          </div>
          <div className="account-section--Home&Living">
            <h2>Home & Living</h2>
            <ul>
              <li>
                <p>Bedding</p>
              </li>
              <li>
                <p>Bathroom</p>
              </li>
              <li>
                <p>Kitchen</p>
              </li>
              <li>
                <p>Dining</p>
              </li>
              <li>
                <p>Living Room</p>
              </li>
              <li>
                <p>Office</p>
              </li>
              <li>
                <p>Kids Room</p>
              </li>
              <li>
                <p>Outdoor</p>
              </li>
              <li>
                <p>Storage</p>
              </li>
              <li>
                <p>Furniture</p>
              </li>
              <li>
                <p>Lighting</p>
              </li>
              <li>
                <p>Window Treatments</p>
              </li>
              <li>
                <p>Home Accessories</p>
              </li>
            </ul>
          </div>

          <div className="account-section-Beauty">
            <h2>Beauty</h2>
            <ul>
              <li>
                <a href="#">Makeup</a>
              </li>
              <li>
                <a href="#">Skin Care</a>
              </li>
              <li>
                <a href="#">Hair Care</a>
              </li>
              <li>
                <a href="#">Nail Care</a>
              </li>
              <li>
                <a href="#">Fragrance</a>
              </li>
              <li>
                <a href="#">Personal Care</a>
              </li>
              <li>
                <a href="#">Tools & Accessories</a>
              </li>
            </ul>
          </div>
        </main>
        <h5 id="myntra-app">Amazon APP</h5>
        <br></br>
        <p>
          Amazon, India’s no. 1 online fashion destination justifies its fashion
          relevance by bringing something new and chic to the table on the
          daily. Fashion trends seem to change at lightning speed, yet the
          Amazon shopping app has managed to keep up without any hiccups. In
          addition, Amazon has vowed to serve customers to the best of its
          ability by introducing its first-ever loyalty program, The Amazon
          Insider. Gain access to priority delivery, early sales, lucrative
          deals and other special perks on all your shopping with the Amazon
          app. Download the Amazon app on your Android or IOS device today and
          experience shopping like never before!
        </p>
        <h5 id="myntra-app">HISTORY OF Amazon</h5> <br></br>
        <p>
          Becoming India’s no. 1 fashion destination is not an easy feat.
          Sincere efforts, digital enhancements and a team of dedicated
          personnel with an equally loyal customer base have made Amazon the
          online platform that it is today. The original B2B venture for
          personalized gifts was conceived in 2007 but transitioned into a
          full-fledged ecommerce giant within a span of just a few years. By
          2012, Amazon had introduced 350 Indian and international brands to its
          platform, and this has only grown in number each passing year. Today
          Amazon sits on top of the online fashion game with an astounding
          social media following, a loyalty program dedicated to its customers,
          and tempting, hard-to-say-no-to deals. The Amazon shopping app came
          into existence in the year 2015 to further encourage customers’
          shopping sprees. Download the app on your Android or IOS device this
          very minute to experience fashion like never before
        </p>
        <footer>
          <p>&copy; 2022 Amazon. All rights reserved.</p>
        </footer>
      </div>
    </div>
  );
}

export default Fotter;
